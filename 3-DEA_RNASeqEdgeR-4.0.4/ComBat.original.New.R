...
ComBat <- function(expression_xls, targets, type='txt', write=T, covariates='all', par.prior=T, filter=F, skip=0, prior.plots=T, plot.to=getwd()){

	message('ComBat: Reading Sample Information File\n')
	saminfo <- targets

	if(sum(colnames(saminfo)=="Batch")!=1){
		message('ComBat ERROR: Sample Information File does not have a Batch column!\n')
	...
	if(filter){
		...
		message('ComBat: Filtered genes absent in more than',filter,'of samples. Genes remaining:',nrow(dat),'; Genes filtered:',ngenes-nrow(dat),'\n')
	}
	
	if(any(apply(dat,2,mode)!='numeric')){
		message('ComBat ERROR: Array expression columns contain non-numeric values! (Check your .xls file for non-numeric values and if this is not the problem, make a .csv file and use the type=csv option)\n')
	...
	if(any(is.na(tmp))){
		message('ComBat ERROR: Sample Information File and Data Array Names are not the same!\n')
		return('ERROR: Sample Information File and Data Array Names are not the same!')
	}

	saminfo <- saminfo[tmp,]  ## Bug fixed 01/04/2011		
	
	if(any(covariates != 'all')){saminfo <- saminfo[,c(1,covariates)]}
	message('ComBat: Build Batch Design Matrix\n')
	...
	NAs = any(is.na(dat))
	if(NAs){message(c('ComBat: Found',sum(is.na(dat)),'Missing Data Values\n'),sep=' ')}
	
	##Standardize Data across genes
	message('ComBat: Standardizing Data across genes\n')
	...
	message("ComBat: Fitting L/S model and finding priors\n")
	...
	if (prior.plots & par.prior){
		png(paste(plot.to,"/BatchEffect.QC.png",sep=""),height=600,width=800)
		par(mfrow=c(2,2))
	...
	if(par.prior){
		message("ComBat: Finding parametric adjustments\n")
		for (i in 1:n.batch){
		...
		message("ComBat: Finding nonparametric adjustments\n")
		for (i in 1:n.batch){
	...
	### Normalize the Data ###
	message("ComBat: Adjusting the Data\n")
	...
	if(write){
		output_file <- sub(".txt","_Adjusted.txt",expression_xls)
		print(output_file)
		print(geneinfo[1:2])
		print(bayesdata[1:2,1:4])
		#cat(c(colnames(geneinfo),colnames(dat),'\n'),file=output_file,sep='\t')
		#write.table(cbind(geneinfo,formatC(as.matrix(bayesdata), format = "f")), file=output_file, sep="\t", quote=F,row.names=F,col.names=F,append=T)
		...
		message("ComBat: Adjusted data saved in file:",output_file,"\n")
...
design.mat <- function(saminfo){
	...
	print(colnames(saminfo))
	tmp1 <- as.factor(saminfo[,tmp])
	message("ComBat: Found",nlevels(tmp1),'batches\n')
	sink(stderr())
	print(tmp1)
	sink()
	design <- build.design(tmp1,start=1)
	ncov <- ncol(as.matrix(saminfo[,-c(1,tmp)]))
	message("ComBat: Found",ncov,'covariate(s)\n')
	if(ncov>0){
		for (j in 1:ncov){
